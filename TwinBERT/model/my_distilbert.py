...
class TransformerBlock(DistilBertTransfomerBlockAdaptersMixin, nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, output_attentions=False):
        ...
        if output_attentions:
            sa_output, sa_weights, q, k, v = sa_output  # (bs, seq_length, dim), (bs, n_heads, seq_length, seq_length)
        ...
        output = (ffn_output,)
        if output_attentions:
            output = (sa_weights, q, k, v) + output
        return output
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                ## add q,k,v as temp value
                assert len(layer_outputs) == 5
                attentions = layer_outputs[1:4]
...
class DistilBertModel(DistilBertModelAdaptersMixin, DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        try:
            ...
            self.resizelayer = nn.Linear(self.statedim, config.dim, bias=False)
        except:
            self.statedim = False

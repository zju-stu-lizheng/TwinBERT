...
from transformers.adapters.mixins.distilbert import (
...
class DistilBertModel(DistilBertModelAdaptersMixin, DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        ## state one-hot vector --resize--> config.dim(768)
        self.resizelayer = nn.Linear(config.statedim, config.dim, bias=False)
    ...
    def forward(
        ...
        ## add one-hot state
        inputs_embeds = self.resizelayer(state_vector) / 4 + inputs_embeds
...
from transformers.adapters.composition import adjust_tensors_for_parallel
from transformers.adapters.context import ForwardContext
from transformers.adapters.mixins.distilbert import DistilBertModelAdaptersMixin, DistilBertTransfomerBlockAdaptersMixin
...
class DistilBertModel(DistilBertModelAdaptersMixin, DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = Embeddings(config)  # Embeddings
        self.transformer = Transformer(config)  # Encoder
        ##(05/01) state one-hot vector --resize--> config.dim(768)
        self.resizelayer = nn.Linear(config.statedim, config.dim, bias=False)
        self.stateweight = nn.Parameter(torch.tensor(4.0))
        # self.stateweight = 4
        self._init_adapter_modules()
    ...
    def forward(
        ...
        if state_vector != None:
            ## add one-hot state
            # print("alpha",1.0/self.stateweight)
            # print("inputs_embeds",inputs_embeds) 
            # print("torch.div(self.resizelayer(state_vector.float())",torch.div(self.resizelayer(state_vector.float()),self.stateweight)) 

            
            # y_sum = torch.sum(torch.div(self.resizelayer(state_vector.float()),self.stateweight).absolute())
            # print(x_sum,y_sum)
            inputs_embeds = torch.div(self.resizelayer(state_vector.float()),self.stateweight) + inputs_embeds
        inputs_embeds = self.invertible_adapters_forward(inputs_embeds)
